...
async function sendMainMenu(to, page=0) {
  ...
  const rows = pageCats.map(([key,cat]) => ({
    id:`CAT_${key}`, title:t24(cat.label), description:`${cat.items.length} items`,
  }));
  if (hasMore) rows.push({id:`MENU_PAGE_${page+1}`,title:"➡️ More",description:"See more categories"});
  if (page > 0) rows.push({id:`MENU_PAGE_${page-1}`,title:"⬅️ Previous",description:"Go back"});
  await sendList(to, "🍽️ Kavi Chettinadu",
    "Authentic Chettinad flavours! 🌶️\nSelect a category:",
    "Browse Menu", [{title:"Menu Categories", rows}]
  );
}
...
async function sendCategoryItems(to, catKey, page=0) {
  ...
  const rows = pageItems.map(item => ({
    id:`ITEM_${item.id}`, title:t24(item.name), description:`Rs.${item.price}`
  }));
  ...
  await sendList(to, t24(cat.label), "Select an item to add to cart:",
    "Choose Item", [{title:t24(cat.label), rows}]
  );
}

async function sendQuantitySelect(to, item) {
  await sendText(to, `*${item.name}*\n💰 Price: Rs.${item.price}\n\n📝 *Enter quantity:*\nType a number (1, 2, 3...) and send!`);
  ...
  try {
    ...
    const payLink = await rzp.paymentLink.create({
      ...
      customer:{
        ...
        contact: normalizePhone(session.deliveryData?.phone||from),
      },
...
async function placeOrder(from, session) {
  if (!session.cart || session.cart.length === 0) {
    console.log("⚠️ placeOrder blocked — cart empty:", from);
  ...
  if (!session.deliveryData || !session.deliveryData.grand_total) {
    console.log("⚠️ placeOrder blocked — no deliveryData:", from);
  ...
  const {name, phone, address, order_type, paymentMethod, addons,
    addon_total, delivery_charge, gst_amount, grand_total,
    table_persons, table_date, table_time, table_seating,
    pickup_time, special_instructions} = session.deliveryData;
...
Browse our menu:`,
    ...
    if (input==="__FLOW_COMPLETE__") {
      ...
      const {
        ...
        delivery_address, pincode, order_type,
        table_persons, table_date, table_time, table_seating,
      ...
      const full_address =
        order_type==="delivery"
          ? liveLocation
            ? `${delivery_address ? delivery_address+", " : ""}📍 ${liveLocation}${pincode?" - "+pincode:""}`
    ...
    if (["PAY_COD","PAY_UPI","PAY_CARD","PAY_REST"].includes(input)) {
      if (!session.deliveryData?.grand_total) {
        console.log("⚠️ Payment blocked — no active order | State:", session.state);
